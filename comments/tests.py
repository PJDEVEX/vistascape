...
class CommentListViewTests(APITestCase):
    ...
    def test_can_list_comments(self):
        ...
        Comment.objects.create(owner=self.user, post=self.post, content='Test comment')
    ...
    def test_unauthenticated_user_cannot_create_comment(self):
        self.client.logout()
        response = self.client.post('/comments/', {'content': 'New comment'})
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
    
    def test_authenticated_user_can_create_comment(self):
        self.client.force_authenticate(user=self.user)
        response = self.client.post('/comments/', {'content': 'New comment', 'post': self.post.id})       
...
class CommentDetailViewTests(APITestCase):
    def setUp(self):
        self.user = User.objects.create_user(username='testuser', password='testpass')
        self.post = Post.objects.create(title='Test Post', content='Test content', owner=self.user)
        self.comment = Comment.objects.create(owner=self.user, post=self.post, content='Test comment')
    ...
    def test_authenticated_user_cannot_update_other_user_comment(self):
