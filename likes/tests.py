...
class LikeViewsTestCase(TestCase):
    def setUp(self):
        ...
        self.other_post = Post.objects.create(title='Other Post', content='Other content', owner=self.other_user)
        self.client = APIClient()
        self.client.force_authenticate(user=self.user)
        self.like = Like.objects.create(owner=self.user, post=self.post)
        self.like_other_user = Like.objects.create(owner=self.other_user, post=self.other_post)
    
    def test_create_like_view(self):
        response = self.client.post('/likes/', {'post': self.post.id}, format='json')
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)

        self.assertEqual(response.data.get('detail'), 'possible duplicate')
    
    def test_create_like_view_success(self):
        new_post = Post.objects.create(title='Another Post', content='Another content', owner=self.user)
        response = self.client.post('/likes/', {'post': new_post.id}, format='json')
    ...
    def test_delete_other_user_like(self):
        response = self.client.delete(f'/likes/{self.like_other_user.id}/')
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
        self.assertTrue(Like.objects.filter(id=self.like_other_user.id).exists())
